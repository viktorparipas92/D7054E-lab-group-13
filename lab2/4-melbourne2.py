...
def plot_council_year():
    council_year = melbourne_data.groupby(
        ['CouncilArea', 'YearBuilt']
    ).size().reset_index(name='count')
    council_year = council_year[council_year['YearBuilt'] >= 1800]

    top10_councils = council_year.groupby(
        'CouncilArea'
    )['count'].sum().nlargest(10).index
    council_year = council_year[
        council_year['CouncilArea'].isin(top10_councils)
    ]

    sns.scatterplot(
        x='YearBuilt', y='count', hue='CouncilArea', data=council_year
    )
...
def plot_seller_location():
    ...
    for i, seller in enumerate(top_sellers):
        ...
        plt.scatter(
            ...
            alpha=0.5, label=seller, cmap='viridis',
        )
